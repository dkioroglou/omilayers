...
class TestDuckdbEngine(unittest.TestCase):
    ...
    def test_16_rename_layer(self):
        omi = Omilayers(self.db, engine=self.engine)
        omi.layers.rename("first_layer", "renamed_layer")
        self.assertFalse(self._dbutils._table_exists('first_layer'))
        self.assertTrue(self._dbutils._table_exists('renamed_layer'))

    def test_17_delete_layer(self):
        omi = Omilayers(self.db, engine=self.engine)
        omi.layers.drop("renamed_layer")
        self.assertFalse(self._dbutils._table_exists('first_layer'))
        layersNames = self._dbutils._get_tables_names()
        self.assertNotIn("renamed_layer", layersNames)
